...
<body>
  ...
  <script>
    ...
    function renderProducts(){
      ...
      document.getElementById("products").innerHTML = shown.map(p => '<article class="card"><img src="'+escapeAttr(p.image)+'" alt=""><div class="card-body"><span class="badge">'+escapeHtml(p.badge || "HookTea")+'</span><h3>'+escapeHtml(p.name)+'</h3><p>'+escapeHtml(p.subtitle || "")+'</p><div><span class="price">$'+money(p.price)+'</span>'+(p.originalPrice && p.originalPrice > p.price ? '<span class="original">$'+money(p.originalPrice)+'</span>' : '')+'</div><button class="buy" onclick="addToCart(\\''+escapeAttr(p.id)+'\\')">加入購物車</button>'+(p.sourceUrl ? '<a class="source-link" target="_blank" rel="noopener" href="'+escapeAttr(p.sourceUrl)+'">查看官網商品</a>' : '')+'</div></article>').join("") || '<div class="empty">目前沒有商品</div>';
    }
