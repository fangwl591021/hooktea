...
export async function reconcileTrackingIncidents(env) {
  ...
  for(const {lease_id:id} of rows)await env.DB.batch([
    durableAlertStatement(env,`tracking:${id}`,'tracking','tracking_unresolved',id,
      'NOT EXISTS(SELECT 1 FROM crm_request_reviews WHERE lease_id=?)',[id]),
    env.DB.prepare(`INSERT OR IGNORE INTO crm_request_reviews(lease_id,status,evidence)
      VALUES(?,'known_unresolved','unconfirmed_finish; no_replay_no_balance_change')`).bind(id),
  ]);
  ...
  for(const {lease_id:id} of finished)await env.DB.batch([
    durableAlertStatement(env,`tracking-recovered:${id}`,'tracking','tracking_recovered',id),
    env.DB.prepare("UPDATE crm_request_reviews SET status='resolved',evidence='request_finish_recorded; business_result_not_replayed',reviewed_at=CURRENT_TIMESTAMP WHERE lease_id=?").bind(id),
  ]);
}
