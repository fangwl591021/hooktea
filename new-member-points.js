...
export function createNewMemberPointService({db,mother}) {
  const stmt=(sql,...args)=>db.prepare(sql).bind(...args);
  const first=(sql,...args)=>stmt(sql,...args).first();
  const wallet=uid=>first('SELECT * FROM child_point_wallets WHERE line_uid=?',uid);
  ...
  async function account(uid,member) {
    ...
    if(row && (row.member_id!==member.userId || member.legacyMemberId ||
  ...
  async function submit(input,member) {
    ...
    if(input.memberUid!==a.member_id)throw Error('POINT_IDENTITY_CONFLICT');
  ...
  async function history(member,page,perPage) {
    const a=await account(member.userId,member);
    ...
    return {balance:summary.results[0].balance,total:summary.results[0].total,records:entries.results.map(e=>({
      id:e.operation_id,get_point:e.amount,point_balance:e.balance_after,event_content:e.reason,created_at:e.created_at}))};
  }
  return {enroll,wallet,get,read,submit,attempt,resume,snapshot,publicResult,history};
}
